...
export default function VideosTab({ accessToken, showToast }) {
  ...
  const retry = async (id) => {
    ...
    try {
      ...
      showToast?.(`Retrying (${d.renderer || 'render'})…`, 'success');
      await load();
    } catch (e) { showToast?.(e.message || 'Error', 'error'); }
  ...
  const copy = (url) => {
    try { navigator.clipboard.writeText(url); showToast?.('Link copied', 'success'); }
    catch { showToast?.('Could not copy', 'error'); }
  };

  const dismiss = async (id) => {
    setBusy(id);
    try { await call({ action: 'dismiss', id }); showToast?.('Removed from problems', 'success'); await load(); }
    catch (e) { showToast?.(e.message || 'Error', 'error'); }
    finally { setBusy(null); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-sm text-gray-400">
            <span className="text-emerald-400 font-semibold">{c.completed ?? 0}</span> ready ·
            <span className="text-amber-400 font-semibold"> {c.processing ?? 0}</span> processing ·
            <span className="text-rose-400 font-semibold"> {c.failed ?? 0}</span> failed ·
            <span className="text-gray-400"> {c.pending ?? 0} no photos</span>
          </p>
        </div>
      ...
      <div className="flex gap-2">
        ...
        />
        <button onClick={search} className="px-4 py-2 rounded-lg bg-gray-700 hover:bg-gray-600 text-white text-sm font-semibold">Search</button>
      ...
      {results !== null && (
        <div className="space-y-2">
          {results.length === 0 ? <Empty text="No results." /> : results.map((v) => <Row key={v.id} v={v} onCopy={copy} onRetry={retry} onView={setModalUrl} busy={busy} />)}
        </div>
      )}
      ...
      {results === null && (
        <section>
          <h3 className="text-xs uppercase tracking-wider text-rose-400 font-semibold mb-3">⚠️ Problems ({(data.problems || []).length})</h3>
          {(data.problems || []).length === 0
            ? <Empty text="No videos with problems. 🎉" />
      ...
      {results === null && (
        <section>
          <h3 className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-3">Recently ready</h3>
...
function StatusPill({ status }) {
  const map = {
    completed: ['Ready', 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30'],
    processing: ['Processing', 'bg-amber-500/15 text-amber-300 border-amber-500/30'],
    photos_uploaded: ['Queued', 'bg-amber-500/15 text-amber-300 border-amber-500/30'],
    failed: ['Failed', 'bg-rose-500/15 text-rose-300 border-rose-500/30'],
    pending: ['No photos', 'bg-gray-600/30 text-gray-300 border-gray-600/40'],
  };
...
function RendererBadge({ renderer }) {
  if (renderer === 'inhouse') {
    return <span className="text-[11px] px-2 py-0.5 rounded-full border bg-sky-500/15 text-sky-300 border-sky-500/30" title="Rendered by our own engine (no Shotstack cost)">🏠 In-house</span>;
  }
  if (renderer === 'shotstack') {
    return <span className="text-[11px] px-2 py-0.5 rounded-full border bg-gray-600/30 text-gray-300 border-gray-600/40" title="Rendered by Shotstack">Shotstack</span>;
  }
  return null;
}
    ...
    <div className="rounded-xl border border-gray-800 bg-[#1a1f26] p-3 flex items-center justify-between gap-3">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          ...
          {v.photo_count ? <span className="text-[11px] text-gray-500">{v.photo_count} photos</span> : null}
        </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {v.video_url && (
          <>
            <button onClick={() => onView?.(v.video_url)}
              className="px-3 py-1.5 text-xs rounded-lg bg-gray-700 hover:bg-gray-600 text-white font-semibold">View</button>
            <button onClick={() => onCopy(v.video_url)}
              className="px-3 py-1.5 text-xs rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-semibold">Copy link</button>
          </>
        )}
        ...
        {onDismiss && (
          <button onClick={() => onDismiss(v.id)} disabled={busy === v.id} title="Remove from problems"
