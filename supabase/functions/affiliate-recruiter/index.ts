      ...
      ratings: {
        ...
        items: {
          ...
          properties: {
            ...
            suggested_commission: { type: 'integer', description: 'Suggested commission % (usually 20-30).' },
            outreach_draft: { type: 'string', description: 'A warm, personal Spanish DM to send them: compliment, clear value (earn commission promoting a product their audience will love), low-pressure invite to reply. 3-5 sentences. No spammy vibe.' },
          },
...
async function score(cands: any[]): Promise<Record<number, any>> {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      system: 'You recruit affiliates for "Regalos Que Cantan", a US-Hispanic brand selling personalized Spanish songs as gifts (~$30, 20% commission). Score each creator as a partner who could promote personalized song gifts to a Latino/family/música/gifting audience — micro-to-mid influencers with engaged audiences convert best. Draft a warm, genuine Spanish outreach DM for each (no spam). Score honestly.',
