...
async function ensureNameInLyrics(opts: {
  ...
  const sys = `Eres editor de letras. La letra que te doy NO menciona el nombre del protagonista y DEBE mencionarlo. Inserta el nombre "${recipientName}" (puedes usar solo el primer nombre o el apodo si el nombre completo es largo o difícil de rimar) de forma natural: una vez DENTRO de las primeras 4 líneas cantadas (en el [Hablado] inicial si existe, si no en el [Verso 1]) y, a lo sumo, una vez más en un verso o el puente (MÁXIMO 2 menciones en total). NO lo metas en el coro (se repite y cansa al oyente). ${perspective} Cambia lo MÍNIMO necesario: conserva la estructura, los marcadores de sección, la rima, la métrica y la emoción. No inventes datos nuevos. Entrega llamando a submit_named_lyrics.`;
...
serve(async (req) => {
  ...
  try {
    ...
    const nameInstruction = isForSelf
      ? `REGLA DEL NOMBRE (canción para uno mismo): aunque la canción va en PRIMERA persona, el nombre "${recipientName}" DEBE CANTARSE al menos una vez — no basta con pronombres. El nombre (puedes usar solo el primer nombre o el apodo si el nombre completo es largo o difícil de rimar) DEBE aparecer dentro de las primeras 4 líneas cantadas — en el [Hablado] inicial si existe, o si no, en el [Verso 1] — para que el comprador escuche SU nombre desde el inicio. Luego, a lo sumo, una vez más en un verso o el puente (MÁXIMO 2 menciones en total). MANTÉN EL NOMBRE FUERA DEL CORO (se repite y cansa). Mantén el tono en primera persona ("yo soy ${recipientName}", "aquí anda ${recipientName}", "me dicen..."). NO la conviertas en un regalo para otra persona. PROHIBIDO cantar la canción entera solo con pronombres ("este soy yo", "soy yo") sin decir el nombre ni una vez. NUNCA fuerces el nombre donde rompa el ritmo — si es difícil de rimar, colócalo al INICIO de la línea.`
      : `REGLA DEL NOMBRE: "${recipientName}" DEBE aparecer en las primeras 4 líneas del [Verso 1] — el comprador necesita escuchar el nombre desde el inicio para sentir la personalización. Luego menciónalo SOLO 1 o 2 veces más, en los versos o el puente — MÁXIMO 3 menciones en toda la canción. MANTÉN EL NOMBRE FUERA DEL CORO: el coro se repite varias veces, así que un nombre ahí se vuelve repetitivo y cansa al oyente — el coro debe ser universal y emocional. (Solo se permite el nombre en el coro si fluye de forma totalmente natural Y no es la línea-gancho que se repite; ante la duda, déjalo FUERA.) NUNCA fuerces el nombre donde no fluya rítmicamente — si es difícil de rimar, úsalo al INICIO de la línea.`;
