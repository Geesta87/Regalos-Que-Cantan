...
// ===========================================================================
// AGENT 2 — CREATIVE STUDIO (admin approval + auto-post + content calendar)
// ===========================================================================
// Powers the dashboard's Creative Studio tab: Review queue (approve/reject/edit)
// AND the Calendario (schedule_list / reschedule / set_posting) that manages the
// hands-off auto-poster's [MES-1] queue.
// Auth: admin_users session. Reads: admin + assistant. Mutations: admin only.
...
async function ghlPost(accountId: string, caption: string, mediaUrl: string, kind: string, scheduleDate: string, variant: 'feed' | 'story' = 'feed') {
  ...
  try {
    ...
    if (action === 'list') {
      ...
      const { data, error } = await admin.from('creative_queue')
        ...
        .in('status', statuses).order('created_at', { ascending: false }).limit(body.limit || 60);
      if (error) return json({ success: false, error: error.message }, 500);
      return json({ success: true, role, creatives: data || [] });
    }

    // ─── SCHEDULE_LIST: the auto-post calendar ([MES-1] queue by post date) ─
    if (action === 'schedule_list') {
      const { data, error } = await admin.from('creative_queue')
        .select('id, batch_date, kind, occasion, concept, headline, caption, hashtags, status, media_url, ghl_post_id, posted_at')
        .like('concept', '[MES-1%').in('status', ['ready', 'posted', 'approved'])
        .order('batch_date', { ascending: true }).order('kind', { ascending: true }).limit(400);
      if (error) return json({ success: false, error: error.message }, 500);
      const { data: st } = await admin.from('creative_posting_state').select('enabled').eq('id', 1).single();
      return json({ success: true, role, posting_enabled: st ? !!st.enabled : true, creatives: data || [] });
    }

    // ─── Live promo box: read (admin + assistant) ────────────────────
    if (action === 'get_config') {
      const { data: cfg } = await admin.from('creative_studio_config').select('promo_notes, style_notes, promo_updated_at').eq('id', 1).single();
      // promo_updated_at drives the "this push is N days old" warning in the UI.
      // A push nobody has touched in weeks quietly steers every generator — that
      // is how a June 4th-of-July brief was still writing ads in mid-August.
      return json({ success: true, role, promo_notes: cfg?.promo_notes || '', style_notes: cfg?.style_notes || '', promo_updated_at: cfg?.promo_updated_at || null });
    }
    ...
    if (action === 'save_promo') {
      ...
      // Stamp promo_updated_at, NOT just updated_at — the latter is also bumped by
      // style_notes saves, so it can't answer "how old is this push?".
      const { error } = await admin.from('creative_studio_config').update({ promo_notes: promo, promo_updated_at: now, updated_at: now }).eq('id', 1);
      if (error) return json({ success: false, error: error.message }, 500);
      return json({ success: true, promo_notes: promo, promo_updated_at: now });
    }

    // ─── Mutations (admin only) ────────────────────────────────
    if (role !== 'admin') return json({ success: false, error: 'Admins only' }, 403);

    // ─── Pause / resume the auto-poster (admin only, no id) ───────────
    if (action === 'set_posting') {
      const enabled = !!body.enabled;
      const { error } = await admin.from('creative_posting_state').upsert({ id: 1, enabled, updated_at: new Date().toISOString() });
      if (error) return json({ success: false, error: error.message }, 500);
      return json({ success: true, posting_enabled: enabled });
    }

    if (!body.id) return json({ success: false, error: 'Missing id' }, 400);

    if (action === 'reject') {
      const { error } = await admin.from('creative_queue').update({ status: 'rejected', updated_at: new Date().toISOString() }).eq('id', body.id);
      if (error) return json({ success: false, error: error.message }, 500);
      return json({ success: true, id: body.id, status: 'rejected' });
    }

    // ─── Reschedule a post to a different day (admin only) ────────────
    if (action === 'reschedule') {
      const d = String(body.batch_date || '');
      if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return json({ success: false, error: 'Bad date (YYYY-MM-DD)' }, 400);
      const { error } = await admin.from('creative_queue').update({ batch_date: d, updated_at: new Date().toISOString() }).eq('id', body.id);
      if (error) return json({ success: false, error: error.message }, 500);
      return json({ success: true, id: body.id, batch_date: d });
    }

    // ─── Edit the copy (caption/headline/body/hashtags) (admin only) ────
    if (action === 'update') {
      ...
      if (body.hashtags !== undefined) patch.hashtags = Array.isArray(body.hashtags) ? body.hashtags.map((h: string) => String(h).replace(/^#/, '').trim()).filter(Boolean).slice(0, 30) : null;
      const { data, error } = await admin.from('creative_queue').update(patch).eq('id', body.id).select('id, headline, primary_text, caption, hashtags').single();
      if (error) return json({ success: false, error: error.message }, 500);
    ...
    if (action === 'request_changes') {
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/creative-chat`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: authHeader }, body: JSON.stringify({ action: 'tweak', creative_id: body.id, change_instructions: instr }) });
    ...
    if (action === 'approve') {
      ...
      const base = (Number.isFinite(reqTs) && reqTs > Date.now() + 120_000) ? reqTs : (Date.now() + FEED_SCHEDULE_DELAY_SECONDS * 1000);
      const results: Record<string, string | null> = {}; let firstId: string | null = null; const errs: string[] = [];
      for (let i = 0; i < accounts.length; i++) {
        const scheduleDate = new Date(base + i * 30_000).toISOString();
        const r = await ghlPost(accounts[i].id, caption, c.media_url, c.kind, scheduleDate, 'feed');
        results[accounts[i].platform] = r.id; if (r.id && !firstId) firstId = r.id; if (r.error) errs.push(`${accounts[i].platform}:${r.error}`);
      }
      const storyResults: Record<string, string | null> = {}; const storyErrs: string[] = [];
      const storyAccounts = accounts.filter((a: any) => a.platform === 'instagram' || a.platform === 'facebook');
      const storyBase = base + 120_000;
      for (let i = 0; i < storyAccounts.length; i++) {
        const scheduleDate = new Date(storyBase + i * 30_000).toISOString();
        const r = await ghlPost(storyAccounts[i].id, caption, c.media_url, c.kind, scheduleDate, 'story');
        storyResults[storyAccounts[i].platform] = r.id; if (r.error) storyErrs.push(`story/${storyAccounts[i].platform}:${r.error}`);
      }
      const allErrs = [...errs, ...storyErrs];
      if (!firstId) {
        await admin.from('creative_queue').update({ status: 'approved', approved_at: approvedAt, error: allErrs.join(' | ').slice(0, 500), updated_at: approvedAt }).eq('id', c.id);
        return json({ success: false, id: c.id, status: 'approved', posted: false, error: `All feed posts failed: ${errs.join(' | ')}` }, 502);
      }
      await admin.from('creative_queue').update({ status: 'posted', approved_at: approvedAt, posted_at: new Date().toISOString(), ghl_post_id: firstId, error: allErrs.length ? allErrs.join(' | ').slice(0, 500) : null, updated_at: new Date().toISOString() }).eq('id', c.id);
      await admin.from('agent_runs').insert({ agent: 'creative-studio', status: 'ok', ok: true, summary: `Approved + posted creative ${c.id} (${c.kind}) — feed + story`, payload: { id: c.id, results, story_results: storyResults }, finished_at: new Date().toISOString() }).then(() => {}, () => {});
      return json({ success: true, id: c.id, status: 'posted', posted: true, ghl_post_id: firstId, results, story_results: storyResults });
    }
