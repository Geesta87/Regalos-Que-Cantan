...
const FIX_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      infill_end_s: {
        type: 'number',
        description: 'End time, in seconds, of the slice to regenerate. End at the END of the last full lyric line of the passage (just after its last word) — never mid-line. Choose a whole phrase, not just the wrong word: aim for a TIGHT 10-12 second window covering the complete line(s) the error sits in, ending on a natural breath — tight windows come back SURGICAL (the rest of the song untouched); windows much wider than that usually make the model re-render the whole song and mutate structure outside the window. Hard limits: 10-60 seconds and no more than half the song duration — a window shorter than 10 seconds is rejected outright by the music API.',
      },
      section_text: {
        type: 'string',
...
- La ventana DEBE durar entre 10 y 60 segundos y NO más de la mitad de la canción.
- REGLA CLAVE 3 (ventana quirúrgica, 2026-08-26): las ventanas de ~10-12s que terminan en un respiro natural regresan QUIRÚRGICAS — el resto de la canción queda intacto. Las ventanas anchas casi siempre regresan RE-RENDERIZADAS: el modelo vuelve a cantar la canción entera y puede duplicar o perder secciones FUERA de la ventana (así se envió una línea duplicada a un cliente que ya pagó). Prefiere SIEMPRE la ventana más corta que cubra la(s) línea(s) del error; ensancha solo si el filtro de copyright (REGLA CLAVE 1) lo exige.
