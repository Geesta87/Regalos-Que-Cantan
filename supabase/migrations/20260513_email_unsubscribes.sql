-- 20260513_email_unsubscribes.sql
...
-- getting abandoned-cart drips, 30-min reminders, etc. That's a CAN-SPAM /
-- CASL / GDPR exposure plus a deliverability liability (repeated complaints
-- against hola@ tank sender reputation for ALL flows, including the order
-- confirmations we just rewrote).
--
...
-- SendGrid bounce/complaint webhook. Marketing emails (checkout recovery,
-- abandoned cart, 30-min reminder, 3-day follow-up) MUST query this before
-- sending. Transactional emails (purchase confirmation, preview-ready,
-- self-service recovery, affiliate welcome credentials) are NOT gated on
-- this table — opt-out customers still receive the product they paid for.
--
...
COMMENT ON TABLE email_unsubscribes IS
  'Email-suppression list for marketing/drip emails. Marketing senders must check this before sending; transactional senders (purchase confirmation, preview-ready) do not.';

COMMENT ON COLUMN email_unsubscribes.source IS
  'How the unsubscribe was recorded: one-click | web | admin | sendgrid-bounce | sendgrid-spamreport | sendgrid-dropped | manual.';
