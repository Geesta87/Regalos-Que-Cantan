...
--
--   1. analyst_readonly is a dedicated role with SELECT-only grants.
--   2. analyst_run_sql() switches to that role (SET LOCAL ROLE) before
--      executing, so even SQL that sneaks past the checks physically cannot
--      write — Postgres itself rejects it.
--   3. Belt-and-suspenders text checks: must start with SELECT/WITH, no write
--      or DDL keywords, single statement only.
...
--
-- Only service_role may execute the function (the edge function's admin gate
-- sits in front of that). anon/authenticated get nothing.
